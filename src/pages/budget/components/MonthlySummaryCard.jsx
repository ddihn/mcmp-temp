        ...
        <table className="table table-bordered">
          ...
          <tbody>
            {data.categories.map((month, idx) => {
              ...
              const analysis = analyzeBudgetPerformance(data.budget[idx], data.actual[idx]);

              return (
                <tr key={month}>
                  <td>{month}</td>
                  <td className="text-end">{currencySymbol}{budget.toLocaleString()}</td>
                  <td className="text-end">{currencySymbol}{actual.toLocaleString()}</td>
                  <td className={`text-end ${analysis.isOverBudget ? 'text-danger' : 'text-success'}`}>
                    {analysis.difference >= 0 ? '+' : ''}{currencySymbol}{convertValue(Math.abs(analysis.difference)).toLocaleString()}
                  </td>
                  <td className={`text-end ${analysis.achievement > 100 ? 'text-danger' : 'text-success'}`}>
